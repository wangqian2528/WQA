...
#define APP_CFG_TASK_CLI_STK_SIZE 2048u
...
static void AppTaskCli(ULONG thread_input);
...
static void AppTaskCreate(void)
{
    ...
    tx_thread_create(&AppTaskFileXTCB,            /* 任务控制块地址 */
                     "App FILEX TEST",            /* 任务名 */
                     AppTaskFileX,                /* 启动任务函数地址 */
                     0,                           /* 传递给任务的参数 */
                     &AppTaskFileXStk[0],         /* 堆栈基地址 */
                     APP_CFG_TASK_FILEX_STK_SIZE, /* 堆栈空间大小 */
                     APP_CFG_TASK_FILEX_PRIO,     /* 任务优先级*/
 ...
 * @brief      os任务信息命令
 */
static int do_show_os_info(uint32_t argc, char *argv[])
{
    DispTaskInfo();
    return 1;
}
cmd_register("os_info", do_show_os_info, "list all os task info");
