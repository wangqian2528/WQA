...
u16 W25QXX_ReadID(void)
{
    u8 temp[2];
    u16 deviceid;
    if(W25QXX_QPI_MODE)QSPI_Send_CMD(W25X_JedecDeviceID, 0, (3 << 6) | (2 << 4) | (3 << 2) | (3 << 0), 0); //QPI,读id,地址为0,4线传输数据_24位地址_4线传输地址_4线传输指令,无空周期,2个字节数据
    else QSPI_Send_CMD(W25X_JedecDeviceID, 0, (1 << 6) | (2 << 4) | (1 << 2) | (1 << 0), 0);            //SPI,读id,地址为0,单线传输数据_24位地址_单线传输地址_单线传输指令,无空周期,2个字节数据
    QSPI_Receive(temp, 2);
...
{
    QSPI_Send_CMD(W25X_FastRead, ReadAddr, (3 << 6) | (3 << 4) | (3 << 2) | (3 << 0), 8);    //QPI,快速读数据,地址为ReadAddr,4线传输数据_32位地址_4线传输地址_4线传输指令,8空周期,NumByteToRead个数据
